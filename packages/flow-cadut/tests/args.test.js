...
describe("type resolvers", () => {
  test("basic type - Address", () => {
    ...
    for (let i = 0; i < cases.length; i++) {
      ...
      const passedArgument = isBasicNumType(type)
        ? argInput.toString()
        : argInput;

      const asArgument = output.asArgument(passedArgument);
...
describe("mapArgument", () => {
  test("Int", async () => {
    ...
    expect(output.value).toBe(input.toString());
  });
  test("UInt", async () => {
    const type = "UInt";
    const input = 42;
...
describe("complex example", () => {
  ...
  test("multiple values from code - shall fail conversion", async () => {
    ...
    await expect(invoke(["42"])).rejects.toThrowError("Not enough arguments");

    // Incorrect integer value provided
    // TODO: fix test or  underlying code
    /*    await expect(
      invoke([0.123, "0x01", ["Hello"], "1.337"])
    ).rejects.toThrowError("Type Error: Expected Integer for type Int");*/

    // Incorrect address
    await expect(invoke([42, true, ["Sup"], "1.337"])).rejects.toThrowError(
      "address.replace is not a function"
    );
    await expect(invoke([42, 12, ["Sup"], "1.337"])).rejects.toThrowError(
      "address.replace is not a function"
    );
    await expect(
      invoke([42, [1, 2, 3], ["Sup"], "1.337"])
    ).rejects.toThrowError("address.replace is not a function");
...
describe("mapValuesToCode", () => {
  test("nested array", async () => {
    ...
    `;
    // TODO: Fix test and underlying code
